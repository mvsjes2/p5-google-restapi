...
sub api {
  ...
  } while($package && $package =~ m|Net::Google::RestApi|);
  $p->{caller} = {
    package => $package,
    line    => $line,
  };
...
Net::Google::RestApi - Oauth2 connection to Google APIs (currently Drive and Sheets).

=head1 SYNOPSIS

=over

  use Net::Google::RestApi;
  $rest_api = Net::Google::RestApi->new(
    login => {
  ...
  use Net::Google::RestApi::SheetsApi4;
  $sheets_api = Net::Google::RestApi::SheetsApi4->new(api => $rest_api);
  $sheet = $sheets_api->open_spreadsheet(title => "payroll");

  use Net::Google::RestApi::DriveApi3;
  $drive = Net::Google::RestApi::DriveApi3->new(api => $rest_api);
...
For specific use of this class, see:
 ...
 Net::Google::RestApi::SheetsApi4
 Net::Google::RestApi::DriveApi3
